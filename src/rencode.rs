...
impl<'a> ser::SerializeSeq for &'a mut RencodeSerializer {
    ...
    fn end(self) -> Result<()> {
        if self.1.pop().unwrap() >= LIST_COUNT {
            self.0.write_u8(types::TERM).unwrap();
        }
        Ok(())
    }
}
...
impl<'a> ser::Serializer for &'a mut RencodeSerializer {
    ...
    fn serialize_unit(self) -> Result<()> {
        self.0.write_u8(types::NONE).unwrap();
        Ok(())
    }
    ...
    fn serialize_bool(self, v: bool) -> Result<()> {
        self.0.write_u8(if v { types::TRUE } else { types::FALSE }).unwrap();
        Ok(())
    }
    ...
    fn serialize_i8(self, v: i8) -> Result<()> {
        match v {
            0..=INT_POS_MAX => {
                self.0.write_i8(INT_POS_START + v).unwrap();
            }
            INT_NEG_MIN..=-1 => {
                self.0.write_i8(INT_NEG_START - 1 - v).unwrap();
            }
            _ => {
                self.0.write_u8(types::INT1).unwrap();
                self.0.write_i8(v).unwrap();
            }
        }
        Ok(())
    }
    ...
    fn serialize_i16(self, v: i16) -> Result<()> {
        self.0.write_u8(types::INT2).unwrap();
        self.0.write_i16::<BE>(v).unwrap();
        Ok(())
    }

    fn serialize_i32(self, v: i32) -> Result<()> {
        self.0.write_u8(types::INT4).unwrap();
        self.0.write_i32::<BE>(v).unwrap();
        Ok(())
    }

    fn serialize_i64(self, v: i64) -> Result<()> {
        self.0.write_u8(types::INT8).unwrap();
        self.0.write_i64::<BE>(v).unwrap();
        Ok(())
    }
    
    fn serialize_f32(self, v: f32) -> Result<()> {
        self.0.write_u8(types::FLOAT32).unwrap();
        self.0.write_f32::<BE>(v).unwrap();
        Ok(())
    }

    fn serialize_f64(self, v: f64) -> Result<()> {
        self.0.write_u8(types::FLOAT64).unwrap();
        self.0.write_f64::<BE>(v).unwrap();
        Ok(())
    }
    ...
    fn serialize_bytes(self, v: &[u8]) -> Result<()> {
        let len = v.len();
        if len < STR_COUNT {
            self.0.write_u8(STR_START + len as u8).unwrap();
        } else {
            self.0.write(format!("{}:", len).as_bytes()).unwrap();
        }
        self.0.write_all(v).unwrap();
        Ok(())
    }
    ...
    fn serialize_tuple(self, len: usize) -> Result<Self::SerializeTuple> {
        if len < LIST_COUNT {
            self.0.write_u8(LIST_START + len as u8).unwrap();
        } else {
            self.0.write_u8(types::LIST).unwrap();
        }
        self.1.push(len);
        Ok(self)
    }
    ...
    fn serialize_map(self, len: Option<usize>) -> Result<Self::SerializeMap> {
        ...
        if len < DICT_COUNT {
            self.0.write_u8(DICT_START + len as u8).unwrap();
        } else {
            self.0.write_u8(types::DICT).unwrap();
        }
        self.1.push(len);
        Ok(self)
    }
